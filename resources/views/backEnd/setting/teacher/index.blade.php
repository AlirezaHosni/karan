                ...
                <div class="row row-sm mt-2">
                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 grid-margin">
                        <div class="card custom-card pt-2">
                            @include('alert.alert')
                            <form action="{{ route('setting.teacherList.store') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="col-12 form-group">
                                    <div class="col-12">
                                        <div class="form-group col-12">
                                            <label class="form-label" for="user_id">کاربر مورد نظر را انتخاب کنید:</label>
                                        ...
                                        <div class="form-group col-12">
                                            ...
                                            <select name="session_id[]"  id="session_id" class="form-control js-example-basic-multiple" multiple="multiple">
                                                @foreach($sessions as $session)
                                                    <option value="{{ $session->id }}">{{ $session->lesson->grade->title }}-{{ $session->lesson->title }}-{{ $session->session }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="row col-12">
                                            <div class="col-6">
                                                <div class="form-group ">
                                                    ...
                                                    <textarea name="resume" id="editor1" rows="9" style="resize: none;">{{ old('resume') }}</textarea>
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <label for="agent_image" class="form-label"> عکس استاد:</label>
                                                <div class="drop-zone col-12">
                                                    <span class="drop-zone__prompt">بارگذاری عکس استاد</span>
