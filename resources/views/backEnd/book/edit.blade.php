                ...
                <div class="row row-sm mt-5">
                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 grid-margin">
                        <div class="card custom-card pt-5">
                            <form action="{{route('book.update', $book->id)}}" method="post" class=" d-flex justify-content-center" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="grade_id"  class="form-control">
                                            @foreach($grades as $grade)
                                                <option value="{{ $grade->id }}" @if($book->lesson->grade_id == $grade->id) selected @endif>{{$grade->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select name="lesson_id"  class="form-control">
                                            @foreach($book->lesson->grade->lessons as $lesson)
                                                <option value="{{ $lesson->id }}" @if($book->lesson_id == $lesson->id) selected @endif>{{ $lesson->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="">عنوان فصل/درس:</label>
                                        <input type="text" name="session" value="{{ old('session', $book->session) }}" class="form-control " placeholder="عنوان فصل/درس" />
                                    </div>
                                    <div class="row">
                                        <div class="form-group col-9">
                                            ...
                                            @if($book->image)
                                            <img src="{{ asset($book->image) }}" style="width: 100px">
                                            @endif
                                        </div>
                                        <div class="col-xl-3">
