                ...
                <div class="row row-sm mt-5">
                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 grid-margin">
                        <div class="card custom-card pt-5">
                            <form action="{{route('lesson.update', $lesson->id)}}" method="post" class=" d-flex justify-content-center" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="grade_id"  class="form-control">
                                            @foreach($grades as $grade)
                                                <option value="{{$grade->id}}" @if(old('grade_id', $lesson->grade_id) == $grade->id) selected @endif>{{$grade->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="">عنوان درس:</label>
                                        <input type="text" name="title" value="{{ old('title', $lesson->title) }}" class="form-control " placeholder="عنوان درس" />
                                    </div>
                                    <label for="description">توضیحات درس:</label>
                                    <div class="form-group">
                                        <textarea name="description" id="description" class="col-12">{{ old('description', $lesson->description) }}</textarea>
                                    </div>
                                    <div class="row">
                                        ...
                                        <div class="col-xl-3">
                                            <div class="form-group row">
                                                <a href="{{ route('lesson.index') }}" class="btn btn-info ml-2 text-white">
                                                    جدید
                                                </a>
