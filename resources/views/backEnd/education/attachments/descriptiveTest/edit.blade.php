                ...
                <div class="row row-sm mt-5">
                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 grid-margin">
                        <div class="card custom-card pt-5">
                                        ...
                                        <div class="form-group col-4">
                                            ...
                                            <select name="grade_id"  class="form-control" id="gradeOptions" data-url="{{ route('searchLessons') }}">
                                                ...
                                                @foreach($grades as $grade)
                                                    <option value="{{ $grade->id }}" @if($descriptiveTest->topic->book->lesson->grade_id == $grade->id) selected @endif>{{$grade->title}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group col-4">
                                            ...
                                            <select name="lesson_id"  class="form-control" id="lessonOptions" data-url="{{ route('searchSessions') }}">
                                                @foreach($descriptiveTest->topic->book->lesson->grade->lessons as $lesson)
                                                    <option value="{{ $lesson->id }}" @if($descriptiveTest->topic->book->lesson_id == $lesson->id) selected @endif>{{ $lesson->title }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group col-4">
                                            ...
                                            <select name="session"  class="form-control" id="sessionOptions" data-url="{{ route('searchParts') }}">
                                                @foreach($descriptiveTest->topic->book->lesson->books()->where('part', null)->get() as $session)
                                                    <option value="{{ $session->session }}" @if($descriptiveTest->topic->book->session == $session->session) selected @endif>{{ $session->session }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                <div class="row-cols-2 row">
                                    <div class="form-group col-6">
                                        ...
                                        <select name="part"  class="form-control" id="partOptions" data-url="{{ route('searchTopics') }}">
                                            @foreach($descriptiveTest->topic->book->lesson->books()->whereNotNull('part')->get() as $part)
                                                <option value="{{ $part->id }}" @if($descriptiveTest->topic->book_id == $part->id) selected @endif>{{ $part->part }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group col-6">
                                        ...
                                        <select name="topic_id" class="form-control" id="topicOptions">
                                            @foreach($descriptiveTest->topic->book->topics as $topic)
                                                <option value="{{ $topic->id }}" @if($descriptiveTest->topic->book_id == $topic->id) selected @endif>{{ $topic->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="card-header border-bottom-0 pb-0">
                                ...
                                <div class="card-body">
                                    <ul class="list-group list-group-horizontal col-12 w-100">
                                        ...
                                        <li class="list-group-item col-3">
                                            <a class="page-link py-10 text-white bg-info" href="{{ route('education.descriptiveTestAttachmentsIndex') }}"> سوالات تشریحی</a>
                                        </li>
                                        <li class="list-group-item col-3">
                            ...
                            <form action="{{ route('education.descriptiveTestAttachments.update', $descriptiveTest->id) }}" method="post" id="form" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                <div class="col-12">
                                    <input type="hidden" name="topic_id" id="selected_topic_id" class="form-control" />
                                @empty(!$descriptiveTest->document)
                                        <div class="col-12 row">
                                            ...
                                            <div class="col-1">
                                                <a class="btn btn-warning btn-sm" href="{{ asset($descriptiveTest->document->document) }}" download>دانلود</a>
                                            </div>
                                            {{--                                            <div class="col-1">--}}
                                            {{--                                                <form action="{{ route('education.IntroduceBookAttachments.destroy', $descriptiveTest->id) }}" method="post">--}}
                                            {{--                                                    @csrf--}}
                                            {{--                                                    @method('delete')--}}
                                    ...
                                    @empty(!$descriptiveTest->video)
                                        <div class="col-12 row">
                                            ...
                                            <div class="col-1">
                                                <a class="btn btn-warning btn-sm" href="{{ asset($descriptiveTest->video->video) }}" download>دانلود</a>
                                            </div>
                                            {{--                                            <div class="col-1">--}}
                                            {{--                                                <form action="{{ route('education.IntroduceBookAttachments.destroy', $descriptiveTest->id) }}" method="post">--}}
                                            {{--                                                    @csrf--}}
